...
def studLogin():
    ...
    try:
        ...
        if not records:
            ...
            #To get the file urls if there is
            ...
            response2 = s3.generate_presigned_url('get_object',
                                                Params={'Bucket': custombucket,
                                                        'Key': file2},
                                                ExpiresIn=expiration)
            response3 = s3.generate_presigned_url('get_object',
                                                Params={'Bucket': custombucket,
                                                        'Key': file3},
                                                ExpiresIn=expiration)
            response4 = s3.generate_presigned_url('get_object',
                                                Params={'Bucket': custombucket,
                                                        'Key': file4},
                                                ExpiresIn=expiration)
            
            return render_template('StudPage.html', student=records, url1=response1, url2=response2, url3=response3, url4=response4, file_exist=True)
...
def studPage():
    ...
    try:
        ...
        try:
            ...
            if companyApForm.filename !="" and parentAckForm.filename !="" and letterOIdt.filename !="" and hiredEvid.filename !="":
                ...
                try:
                    s4.head_object(Bucket=custombucket, Key=file1)
                    file_exists = True
                except ClientError as e:
                    if e.response['Error']['Code'] == '404':
                        file_exists = False
    ...
    return render_template('StudPage.html', student = records, url1=response1, url2=response2, url3=response3, url4=response4, file_exist=True, success=True)
